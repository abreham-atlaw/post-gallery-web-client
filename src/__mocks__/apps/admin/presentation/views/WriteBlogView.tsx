			...
			<form onSubmit={this.handleSubmit} className={"px-6 lg:px-14 py-12"}>
				...
				<div className="w-full pt-10 pb-6 mb-3 border-b-2 border-[#C1C1C1] flex flex-row justify-between">
					<div className="flex flex-row justify-center items-center">
						<p className="text-3xl lg:text-4xl font-bold mr-5">Upload Blog</p>
						<Switcher />
					</div>
		...
		<label className='flex cursor-pointer select-none items-center'>
		  <div className='relative'>
			<input
			  ...
			  className='sr-only'
			/>
			<div className={`box ${isChecked ? 'bg-black' : 'bg-[#00A3FF]'} block h-14 w-32 rounded-full`}>
				<p className={`box ${isChecked ? 'hidden' : 'absolute'} top-4 left-5 text-white`}>Blog</p>
				<p className={`box ${isChecked ? 'absolute' : 'hidden'} top-4 right-5 text-white`}>Press</p>
			</div>
			<div
