...
const App: React.FC<AppProps> = ({artFairs: artFairs, projects: projects}) => {
    return (
        <div >
			<p className='text-2xl font-Lato my-10'>Edit Exhibitions</p>  
          <div className='flex flex-row flex-wrap'>
			{
				projects.map(
					(projects: Publishment) => <Link to={`/admin/project/edit/${projects.getPK()}`} className='font-Lato w-full lg:w-2/5 flex flex-row justify-start items-center h-32 pr-3 mr-5 mb-6 font-medium border-[3px] rounded-md border-[#D6D6D6]'>
			...
			{
				artFairs.map(
					(artFairs: ArtFair) => <Link to={`/admin/fair/edit/${artFairs.getPK()}`} className='font-Lato w-full lg:w-2/5 flex flex-row justify-start items-center h-32 pr-3 mr-5 mb-6 font-medium border-[3px] rounded-md border-[#D6D6D6]'>
